defmodule Cucumber.CucumberExpressions.GeneratorTest do
  ...
  defp assert_expression(registry, expected_expression, expected_argument_names, text) do
    generated = hd(Generator.generate_expressions(registry, text))
  ...
  defp add!(registry, options),
    do: ParameterTypeRegistry.add!(registry, ParameterType.new!(options))
  ...
  test "documents expression generation", %{registry: registry} do
    ...
    generated = hd(Generator.generate_expressions(registry, undefined_step_text))
  ...
  test "exposes parameters in a generated expression", %{registry: registry} do
    generated = hd(Generator.generate_expressions(registry, "I have 2 cukes and 1.5 euro"))
  ...
  test "matches parameter types with optional capture groups", %{registry: registry} do
    registry =
      registry
      |> add!(name: "optional-flight", regexps: ~r/(1st flight)?/)
      |> add!(name: "optional-hotel", regexps: ~r/(1 hotel)?/)

    generated =
      hd(Generator.generate_expressions(registry, "I reach Stage 4: 1st flight -1 hotel"))
  ...
  test "generates at most 256 expressions", %{registry: registry} do
    registry =
      Enum.reduce(0..3, registry, fn i, registry ->
        add!(registry, name: "my-type-#{i}", regexps: ~r/([a-z] )*?[a-z]/)
      end)

    # This would otherwise generate 4^11=4194304 expressions.
    expressions = Generator.generate_expressions(registry, "a s i m p l e s t e p")
  ...
  test "prefers expression with longest non empty match", %{registry: registry} do
    registry =
      registry
      |> add!(name: "zero-or-more", regexps: ~r/[a-z]*/)
      |> add!(name: "exactly-one", regexps: ~r/[a-z]/)

    expressions = Generator.generate_expressions(registry, "a simple step")
  ...
  describe "word boundaries" do
    setup %{registry: registry} do
      {:ok, registry: add!(registry, name: "direction", regexps: ~r/(up|down)/)}
    end

    test "does not suggest a parameter at the beginning of a word", %{registry: registry} do
      assert hd(Generator.generate_expressions(registry, "When I download a picture")).source ==
               "When I download a picture"
    end

    test "does not suggest a parameter inside a word", %{registry: registry} do
      assert hd(Generator.generate_expressions(registry, "When I watch the muppet show")).source ==
               "When I watch the muppet show"
    end

    test "does not suggest a parameter at the end of a word", %{registry: registry} do
      assert hd(Generator.generate_expressions(registry, "When I create a group")).source ==
               "When I create a group"
    end

    test "suggests a parameter for a full word", %{registry: registry} do
      assert hd(Generator.generate_expressions(registry, "When I go down the road")).source ==
               "When I go {direction} the road"

      assert hd(Generator.generate_expressions(registry, "When I walk up the hill")).source ==
               "When I walk {direction} the hill"

      assert hd(Generator.generate_expressions(registry, "up the hill, the road goes down")).source ==
    ...
    test "suggests a parameter wrapped around punctuation characters", %{registry: registry} do
      assert hd(Generator.generate_expressions(registry, "When direction is:down")).source ==
               "When direction is:{direction}"

      assert hd(Generator.generate_expressions(registry, "Then direction is down.")).source ==
